...
async function uploadDocumentFileAPI(filePath, notebookId, additionalContent) {
    return callBridge(window.electronAPI.uploadDocumentFile(filePath, notebookId, additionalContent));
}
...
function askDocumentStreamAPI(documentId, question, useDocumentContext, onChunk, onTokenUsage) {
    return new Promise((resolve, reject) => {
        ...
        const cleanChunk = window.electronAPI.onChatChunk(onChunk);
        
        let finalUsage = null;
        const cleanUsage = window.electronAPI.onChatTokenUsage((usage) => {
            finalUsage = usage;
            if (onTokenUsage) onTokenUsage(usage);
        });

        const cleanEnd = window.electronAPI.onChatEnd(() => {
            cleanAll();
            resolve(finalUsage);
        });

        const cleanError = window.electronAPI.onChatError((err) => {
            cleanAll();
